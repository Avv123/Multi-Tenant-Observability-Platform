...
func main() {
	...
	platformruntime.Start(ctx, processingcache.Get(), platformruntime.HeartbeatOptions{
	...
	switch *mode {
	case "worker":
		if err := telemetryworkers.Run(ctx); err != nil {
...
func runHTTP(ctx context.Context) {
	...
	if err := router.Initialize(ctx, server); err != nil {
		logging.Fatalf("failed to initialize processing router: %v", err)
	}
	go func() {
		<-ctx.Done()
		_ = server.Shutdown(context.Background())
	}()
	logging.Infof("starting processing-service on port %s", config.GetString("server.port"))
	if err := server.Start(); err != nil && err.Error() != "http: Server closed" {
		logging.Fatalf("failed to start processing-service: %v", err)
	}
}
