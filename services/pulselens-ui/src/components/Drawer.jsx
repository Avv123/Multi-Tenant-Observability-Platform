...
export default function Drawer({ open, onClose, title, description, children, width = "440px" }) {
  ...
  return (
    <>
      ...
      <div ref={drawerRef} role="dialog" aria-modal="true" aria-label={title} style={{
        ...
        <div style={{
          ...
          padding: "1.5rem 1.5rem 1rem", borderBottom: "1px solid var(--border)", flexShrink: 0,
        }}>
          <div>
            <h2 style={{ fontSize: "1.05rem", fontWeight: 700, marginBottom: "0.25rem" }}>{title}</h2>
            {description && <p style={{ fontSize: "0.8rem", color: "var(--text-2)", lineHeight: 1.5 }}>{description}</p>}
          </div>
          <button onClick={onClose} aria-label="Close" style={{
            ...
            fontSize: "1rem", flexShrink: 0, marginLeft: "0.75rem", transition: "all 0.15s",
          }}>✕</button>
        </div>
