...
export default function MetricsPage({ state, notify }) {
  ...
  const [filters, setFilters] = useState({ metric_name:"", service_id:"", lookback_minutes:"120" });
  const { data: metrics, loading, error, refetch } = useAsyncData(
    () => queryApi.metricsWithFilters(token, { ...filters, lookback_minutes: parseInt(filters.lookback_minutes)||120 }),
      ...
      <div className="panel" style={{ padding:"1rem" }}>
        <div className="form-grid">
          ...
          <div className="form-group" style={{ marginBottom:0 }}>
            ...
            <input className="form-input" placeholder="e.g. api-gateway" value={filters.service_id} onChange={e=>f("service_id",e.target.value)} />
          </div>
          <div className="form-group" style={{ marginBottom:0 }}>
