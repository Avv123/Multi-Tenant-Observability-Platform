...
const CHANNEL_TYPES = [
  { value: "webhook",       icon: "⚡", label: "Webhook",       desc: "POST JSON to any HTTP endpoint." },
  { value: "slack_webhook", icon: "💬", label: "Slack",          desc: "Send to a Slack channel via Incoming Webhook." },
  { value: "email",         icon: "✉️",  label: "Email",          desc: "Send to one or more email recipients." },
  { value: "stdout",        icon: "📋", label: "Console (log)", desc: "Print to alerting-service stdout — for testing." },
];
...
export default function AlertsPage({ state, notify }) {
  ...
  const [ruleForm, setRuleForm] = useState({
    name:"", signal_type:"log", severity:"error", aggregation:"count",
    comparator:">=", threshold:1, window_minutes:10, cooldown_minutes:5, metric_name:"", policy_id:"",
  });
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:"1.25rem" }}>
      <div>
        ...
        <p style={{ color:"var(--text-2)",fontSize:"0.875rem" }}>Configure detection rules and notification channels. Rules evaluate every 30s.</p>
      </div>
      ...
      <div className="panel">
        <div className="panel-header">
          <div>
            <div className="panel-title">Alert Rules <Tooltip text="Rules continuously evaluate signal data. When the threshold is crossed, an incident is created and notifications dispatched." /></div>
            <div className="panel-desc">Evaluated every 30s by a concurrent goroutine pool</div>
          </div>
          <div style={{ display:"flex",gap:"0.5rem" }}>
            <button className="btn btn-secondary btn-sm" onClick={refetchRules}>↺</button>
        ...
        {rulesLoading ? <SectionLoader /> : (
          rules?.length ? (
            ...
            <EmptyState icon="◆" title="No alert rules" body="Create your first rule to start monitoring signal thresholds."
      ...
      <div className="panel">
        <div className="panel-header">
          <div>
            <div className="panel-title">Notification Channels <Tooltip text="Channels define where alerts are delivered. A channel must be attached to a Policy to receive incidents." /></div>
            <div className="panel-desc">Webhook, Slack, email, or console destinations</div>
          </div>
          <div style={{ display:"flex",gap:"0.5rem" }}>
            <button className="btn btn-secondary btn-sm" onClick={refetchChannels}>↺</button>
        ...
        {channelsLoading ? <SectionLoader /> : (
          channels?.length ? (
                ...
                <tbody>
                  {channels.map(c => (
                    <tr key={c.id}>
                      ...
                      <td><span className="badge badge-info">{CHANNEL_TYPES.find(t=>t.value===c.type)?.icon} {c.type}</span></td>
            ...
            <EmptyState icon="◈" title="No channels" body="Add a webhook, Slack, or email channel to receive incident notifications."
      ...
      <div className="panel">
        <div className="panel-header">
          <div>
            <div className="panel-title">Alert Policies <Tooltip text="Policies govern how incidents are escalated and re-notified. They link alert rules to channels and define retry intervals." /></div>
            <div className="panel-desc">Control escalation, retry, and delivery behaviour</div>
          </div>
          <button className="btn btn-secondary btn-sm" onClick={refetchPolicies}>↺</button>
        </div>
      ...
      {/* ═══ Create Rule Drawer ═════════════════════════════ */}
      <Drawer open={ruleDrawer} onClose={() => setRuleDrawer(false)} title="New Alert Rule" description="Define when PulseLens should create an incident based on your telemetry signals." width="480px">
        <form onSubmit={handleCreateRule} style={{ display:"flex",flexDirection:"column",gap:"1.25rem" }}>
          ...
          <div>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:"0.85rem" }}>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"0.75rem" }}>
                <Field label="Aggregation" tip="How events are counted in the window. 'count' = number of events; 'avg' = mean value of a metric field.">
                ...
                <Field label="Comparator" tip="Operator that compares the aggregated value against the threshold (e.g. '>=' fires when count ≥ threshold).">
                ...
                <Field label="Threshold" tip="The numeric value that triggers the rule when crossed.">
                  <input className="form-input" type="number" value={ruleForm.threshold} onChange={e => rf("threshold",e.target.value)} />
                </Field>
              </div>
              <Field label="Evaluation Window (min)" tip="Rolling time window evaluated each cycle. A 10-minute window with count >= 5 means '5 events in the last 10 minutes'.">
          ...
          <div>
            ...
            <Field label="Cooldown (min)" tip="Minimum time between repeated firings. Prevents alert storms — e.g. 5-minute cooldown = at most one incident per 5 minutes.">
      ...
      {/* ═══ Add Channel Drawer ══════════════════════════════ */}
      <Drawer open={channelDrawer} onClose={() => setChannelDrawer(false)} title="Add Notification Channel" description="Choose how PulseLens delivers incident notifications." width="460px">
        <form onSubmit={handleCreateChannel} style={{ display:"flex",flexDirection:"column",gap:"1.25rem" }}>
          ...
          {(channelForm.type==="webhook"||channelForm.type==="slack_webhook") && (
            <div style={{ display:"flex",flexDirection:"column",gap:"0.75rem" }}>
              <Field label="Webhook URL" tip={channelForm.type==="slack_webhook"?"Use the Incoming Webhook URL from your Slack App.":"Full URL including protocol (https://...)."}>
          ...
          {channelForm.type==="email"&&(
            <Field label="Recipients" tip="Comma-separated list of email addresses.">
              <input className="form-input" placeholder="ops@company.com, alerts@company.com" value={channelForm.email_to} onChange={e=>cf("email_to",e.target.value)} />
            </Field>
          )}
          {channelForm.type==="stdout"&&(
            <div style={{ padding:"0.75rem",background:"var(--surface-active)",borderRadius:"var(--r-sm)",border:"1px solid var(--border)",fontSize:"0.8rem",color:"var(--text-2)" }}>
              ℹ️ Console channels write incident payloads to the <code style={{ background:"rgba(255,255,255,0.06)",padding:"1px 4px",borderRadius:"3px" }}>pulselens-alerting-service</code> stdout. No config needed.
            </div>
          )}
          <div style={{ display:"flex",gap:"0.65rem",paddingTop:"0.5rem",borderTop:"1px solid var(--border)" }}>
