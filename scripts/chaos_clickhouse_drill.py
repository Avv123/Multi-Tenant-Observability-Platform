...
CLICKHOUSE_CONTAINER = "pulselens-clickhouse"
...
def docker(*args):
    subprocess.run(["docker", *args], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)


def bootstrap_token():
    suffix = str(int(time.time()))
    email = f"chaos-clickhouse+{suffix}@pulselens.local"
    req(
        BASE_TENANT + "/internal/api/v1/tenants",
        "POST",
        {
            "name": "Chaos CH Tenant",
            "slug": f"chaos-ch-{suffix}",
            "plan": "starter",
            "ingest_quota": 1000,
            "retention_days": 7,
            "admin_name": "Chaos",
...
def main():
    ...
    before = dependency_row(token, "clickhouse")
    docker("stop", CLICKHOUSE_CONTAINER)
    down = wait_for_status(token, "clickhouse", "down")
    docker("start", CLICKHOUSE_CONTAINER)
    time.sleep(5)
    recovered = wait_for_status(token, "clickhouse", "healthy")
    print(json.dumps({
        "before": before,
        "down": down,
        "recovered": recovered,
